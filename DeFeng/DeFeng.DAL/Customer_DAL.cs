    ...
    public class Customer_DAL
    {
        ...
        public List<Customer> Search(Customer customer)
        {
            ...
            try
            {
                ...
                sql.Append("LEFT JOIN [EntrustOverDate] ON c.entrustOverDate=[EntrustOverDate].ID ");
                ...
                while (result.Read())
                {
                    ...
                    obj.Park = Convert.IsDBNull(result["park"]) ? "" : Convert.ToString(result["park"]);
                    obj.Industry = Convert.IsDBNull(result["industry"]) ? "" : Convert.ToString(result["industry"]);
                    obj.Electricity = Convert.IsDBNull(result["electricity"]) ? "" : Convert.ToString(result["electricity"]);
                    obj.Supporting = Convert.IsDBNull(result["supporting"]) ? "" : Convert.ToString(result["supporting"]);
                    obj.DormCount = Convert.IsDBNull(result["dormCount"]) ? 0 : Convert.ToInt32(result["dormCount"]);  
                    obj.OfficeCount= Convert.IsDBNull(result["officeCount"]) ? 0 : Convert.ToInt32(result["officeCount"]);
                    obj.ClearingCount = Convert.IsDBNull(result["clearingCount"]) ? 0 : Convert.ToInt32(result["clearingCount"]);
                    obj.WorkerCount = Convert.IsDBNull(result["workerCount"]) ? 0 : Convert.ToInt32(result["workerCount"]);
                    obj.EntrustOverDate = new EntrustOverDate
                    {
                        ID = Convert.IsDBNull(result["entrustOverDateID"]) ? 0 : Convert.ToInt32(result["entrustOverDateID"]),
                        Name = Convert.IsDBNull(result["entrustOverDateName"]) ? "" : Convert.ToString(result["entrustOverDateName"])
                    };
        ...
        public int AddCutomer(Customer customer)
        {
            ...
            try
            {
                ...
                sqlPars.Add(new SqlParameter("@shopArea", customer.ShopArea.ID));
                sqlPars.Add(new SqlParameter("@entrustOverDate", customer.EntrustOverDate != null ? customer.EntrustOverDate.ID : 0));
        ...
        public int UpdateCustomer(Customer customer)
        {
            ...
            try
            {
                var sql = "UPDATE Customer SET [customerName]=@customerName,[contacts]=@contacts,[customerPhone]=@customerPhone,[contactsPhone]=@contactsPhone,[customerMobile]=@customerMobile,[IDCard]=@IDCard,[presentAddress]=@presentAddress,[city]=@city,[district]=@district,[area]=@area,[residentialDistrict]=@residentialDistrict,[customerDemand]=@customerDemand,[customerStatus]=@customerStatus,[customerTransactionType]=@customerTransactionType,[isPrivateCustomer]=@isPrivateCustomer,[isQualityCustomer]=@isQualityCustomer,[isPubliceCustomer]=@isPubliceCustomer,[position]=@position,[houseUseType]=@houseUseType,[houseType]=@houseType,[roomCount]=@roomCount,[hallCount]=@hallCount,[toiletCount]=@toiletCount,[balconyCount]=@balconyCount,[houseSizeFrom]=@houseSizeFrom,[houseSizeTo]=@houseSizeTo,[priceFrom]=@priceFrom,[priceTo]=@priceTo,[source]=@source,[grade]=@grade ,[intention]=@intention,shopArea=@shopArea,[nationality]=@nationality,entrustOverDate=@entrustOverDate,[floor]=@floor,[orientation]=@orientation,[decorationType]=@decorationType,[housePayType]=@housePayType,[supporting]=@supporting,[commissionPayType]=@commissionPayType,[entrustType]=@entrustType ,[customerType]=@customerType,[shopLocation]=@shopLocation,[industry]=@industry,[totalFloor]=@totalFloor,[wall]=@wall,[electricity]=@electricity,[park]=@park,[landType]=@landType,[OfficeLevel]=@OfficeLevel,[workerCount]=@workerCount,[dormCount]=@dormCount,[officeCount]=@officeCount,[clearingCount]=@clearingCount,[current]=@current,[carPark]=@carPark,[landPlan]=@landPlan,[remarks]=@remarks,[lastUpdateStaff]=@lastUpdateStaff,[lastUpdateDate]=@lastUpdateDate  WHERE ID=@ID";
                var sqlPars = new List<SqlParameter>();
